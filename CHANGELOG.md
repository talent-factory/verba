...
- **macOS — recording no longer freezes at "Transcribing…" when audio teardown stalls:** The native capture thread's cpal/CoreAudio stream teardown can intermittently hang, and `stop_capture`'s unbounded thread `join()` turned that into a permanent "Transcribing…" freeze with no error and no logs (the transcription step was never reached). The macOS controller now bounds the recording-stop step with a timeout and recovers to idle with a clear error notification instead of freezing. The underlying cpal teardown stall (the trigger) is tracked separately.
