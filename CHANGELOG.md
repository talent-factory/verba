...
- **VS Code Extension — Template picker shows icons and marks the active template:** The `dictation.selectTemplate` Quick Pick (`Cmd+Alt+T`) now renders each template's emoji icon in the label (parity with the macOS tray) and marks the currently active (last-used) template with a check icon in an aligned gutter column — via VS Code's item icon gutter (`ThemeIcon`), so every row lines up like the native macOS menu rather than shifting the active row. Context-aware templates now show a search hint in the item description instead of a leading label icon. The `icon` template field, previously honored only by the macOS tray, is now rendered by the VS Code picker as well.
